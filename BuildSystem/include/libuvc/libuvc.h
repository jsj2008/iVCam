...
typedef enum uvc_error {
  ...
  /** Invalid Data */
  UVC_ERROR_INVALID_DATA = -20,
  /** Camera Data Layout Version Info error or lost */
  UVC_ERROR_CAMERA_DATA_LAYOUT_VERSION = -21,
 ...
 * @ingroup streaming
 * NOTE: new formats MUST add to tail of the list(just before 
 * UVC_FRAME_FORMAT_COUNT), otherwise this will affect those defined enum value 
 * which SHOULD keep the same with upper caller(in JAVA API)
 */
enum uvc_frame_format {
  ...
  /** UVC_FRAME_FORMAT_FRAME_BASED_H264 == 8 currently*/
  UVC_FRAME_FORMAT_FRAME_BASED_H264,
...
// todo:
#define STILL_IMAGE_MAX_COMPRESSION_PATTERN 32
typedef struct uvc_still_image_frame_desc {
  uint8_t bEndpointAddress;
  uint8_t bNumImageSizePatterns;
  uint16_t wWidth[16];
  uint16_t wHeight[16];
  uint8_t bNumCompressionPattern;
  uint8_t bCompression[STILL_IMAGE_MAX_COMPRESSION_PATTERN];
} uvc_still_image_frame_desc_t;
...
typedef struct uvc_format_desc {
  ...
  uvc_still_image_frame_desc_t still_image_frame_descs[16];
  int num_still_image_frame_desc;
} uvc_format_desc_t;
...
uvc_error_t uvc_init(char *usb_syspath, uvc_context_t **ctx, struct libusb_context *usb_ctx);
void uvc_exit(uvc_context_t *ctx);
void uvc_set_debuglog(uvc_context_t *ctx, char *tag, int level);
...
libusb_device *uvc_get_usb_device(uvc_device_t *dev);
...
uvc_error_t uvc_get_devices(uvc_context_t *ctx, uvc_device_t ***devs,
    int *num_devs, int vid, int pid);
...
uvc_error_t uvc_start_streaming(
    uvc_device_handle_t *devh,
    uvc_stream_ctrl_t *ctrl,
    enum uvc_frame_format stillimage_format, int stillimage_width, int stillimage_height,
...
uvc_error_t uvc_start_streaming2(
    ...
    void *user_ptr,
    uint8_t flags,
    uvc_stream_handle_t **ostrmh
);

uvc_error_t uvc_stream_still_image_ctrl_capture(uvc_stream_handle_t *strmh, uvc_stream_ctrl_t *ctrl);

void uvc_pause_streaming(uvc_stream_handle_t *strmh);
uvc_error_t uvc_resume_streaming(uvc_stream_handle_t *strmh);
