...
extern void log_output(FILE *fp, const char *file, int line_num, const char *func_name, const char *format, ...);

#ifdef ANDROID
extern char LogTag[128];
#endif
...
#ifdef UVC_DEBUGGING
#define UVC_DEBUG(...) log_output(stdout, basename(__FILE__), __LINE__, __FUNCTION__, __VA_ARGS__)
#define UVC_ENTER() log_output(stdout, basename(__FILE__), __LINE__, __FUNCTION__, "enter");
#define UVC_EXIT(code) log_output(stdout, basename(__FILE__), __LINE__, __FUNCTION__, "exit: %d", (int)code)
#define UVC_EXIT_VOID() log_output(stdout, basename(__FILE__), __LINE__, __FUNCTION__, "exit")
#else
#define UVC_DEBUG(...)
#define UVC_ENTER()
...
#define UVC_LOG_ERROR(...)  log_output(stderr, basename(__FILE__), __LINE__, __FUNCTION__, __VA_ARGS__)
#define UVC_LOG_INFO(...)  log_output(stdout, basename(__FILE__), __LINE__, __FUNCTION__, __VA_ARGS__)
...
#define LIBUVC_XFER_BUF_SIZE  ( 32 * 1024 )
#define LIBUVC_NUM_XFER_BUF_SIZE 100
...
struct uvc_stream_handle {
  ...
  int32_t sugguest_buf_size;
  struct uvc_buffer *out_buffer;
  struct uvc_buffer_list hold_buffers;
  struct uvc_buffer_pool buffer_pool;
...
#define VAL_RANGE 10000

static inline int si16val_enc(struct si16_val *def, int16_t origin_val)
{
  return VAL_RANGE * (origin_val - def->min_val) / (def->max_val - def->min_val);
}

static inline int16_t i16val_dec(struct si16_val *def, int enc_val)
{
  if(enc_val < 0)
    enc_val = 0;
  if(enc_val > VAL_RANGE)
    enc_val = VAL_RANGE;
  int16_t val1 = (int16_t)round(enc_val * (def->max_val - def->min_val) / (VAL_RANGE * 1.0));
  val1 = val1 / def->res_val * def->res_val;
  return val1 + def->min_val;
}

static inline int ui16val_enc(struct ui16_val *def, uint16_t origin_val)
{
  return VAL_RANGE * (origin_val - def->min_val) / (def->max_val - def->min_val);
}

static inline uint16_t ui16val_dec(struct ui16_val *def, int enc_val)
{
  if(enc_val < 0)
    enc_val = 0;
  if(enc_val > VAL_RANGE)
    enc_val = VAL_RANGE;
  uint16_t val1 = (uint16_t)round(enc_val * (def->max_val - def->min_val) / (VAL_RANGE * 1.0));
  val1 = val1 / def->res_val * def->res_val;
  return val1 + def->min_val;
}

static inline int ui32val_enc(struct ui32_val *def, uint32_t origin_val)
{
  return VAL_RANGE * (origin_val - def->min_val) / (def->max_val - def->min_val);
}

static inline uint32_t ui32val_dec(struct ui32_val *def, int enc_val)
{
  if(enc_val < 0)
    enc_val = 0;
  if(enc_val > VAL_RANGE)
    enc_val = VAL_RANGE;
  uint32_t val1 = (uint32_t)round(enc_val * (def->max_val - def->min_val) / (VAL_RANGE * 1.0));
  val1 = val1 / def->res_val * def->res_val;
  return val1 + def->min_val;
}
...
struct uvc_device_handle {
  ...
  struct si16_val brightness;
  struct ui16_val contrast;
  struct si16_val hue;
  struct ui16_val saturation;
  struct ui16_val gain;
  struct ui16_val sharpness;
  struct ui16_val gamma;
  struct ui16_val white_balance_temperature;

  uint8_t auto_exposure_mode_bitmap;
  struct ui32_val exposure_time_absolute;
